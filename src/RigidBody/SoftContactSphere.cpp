...
rigidbody::SoftContactSphere::SoftContactSphere(const SoftContactNode &other)
    : rigidbody::SoftContactNode(other),
      ...
      m_muViscous(std::make_shared<utils::Scalar>(0.5)),
      m_transitionVelocity(std::make_shared<utils::Scalar>(0.01)) {
  const rigidbody::SoftContactSphere &tp =
      dynamic_cast<const SoftContactSphere &>(other);
...
void rigidbody::SoftContactSphere::setRadius(const utils::Scalar &radius) {
  *m_radius = radius;
}

utils::Scalar rigidbody::SoftContactSphere::radius() const { return *m_radius; }
...
void rigidbody::SoftContactSphere::setDamping(const utils::Scalar &damping) {
  *m_damping = damping;
}
...
void rigidbody::SoftContactSphere::setMuStatic(const utils::Scalar &muStatic) {
  *m_muStatic = muStatic;
}
...
utils::Vector3d rigidbody::SoftContactSphere::computeForce(
  ...
  utils::Vector3d tangentVelocity = dx - normalVelocity * normal +
                                    (*m_radius * normal).cross(angularVelocity);
  ...
  utils::Scalar forceFactor =
      4. / 3. * *m_stiffness * std::sqrt(*m_radius) *
  ...
  utils::Scalar tangentVelocityNorm(
      std::sqrt(tangentVelocity.squaredNorm() + 1e-5));
  utils::Scalar frictionVelocity = tangentVelocityNorm / *m_transitionVelocity;

  // from Peter Brown 2017
  utils::Scalar forceFriction =
      normalForce * *m_muDynamic * std::tanh(4. * frictionVelocity) +
      normalForce * (*m_muStatic - *m_muDynamic) * frictionVelocity /
          ((0.25 * frictionVelocity * frictionVelocity + 0.75) *
           (0.25 * frictionVelocity * frictionVelocity + 0.75)) +
      normalForce * *m_muViscous * tangentVelocityNorm;

  // Total Force
  return normalForce * normal +
         forceFriction * -tangentVelocity / tangentVelocityNorm;
}
