...
void Reader::readModelFile(const utils::Path &path, Model *model) {
  // Open file
  ...
  try {
    while (file.read(
      ...
      if (!main_tag.tolower().compare("segment")) {
        ...
        bool isRangeQSet(false);  // Ranges must be done only after translation
                                  // AND rotations tags
        bool isRangeQdotSet(false);   // Ranges must be done only after
                                      // translation AND rotations tags
        bool isRangeQddotSet(false);  // Ranges must be done only after
                                      // translation AND rotations tags
        while (file.read(property_tag) &&
               property_tag.tolower().compare("endsegment")) {
          if (!property_tag.tolower().compare("parent")) {
            ...
            if (parent_str.tolower().compare("root")) {
              utils::Error::check(
                  model->GetBodyId(parent_str.c_str()),
            ...
            for (size_t i = 0; i < trans.length() + rotLength; ++i) {
              file.read(min);
              file.read(max);
              QRanges.push_back(utils::Range(min, max));
            }
            isRangeQSet = true;
            ...
            file.read(mass, variable);
              ...
              !property_tag.tolower().compare("inertia") ||
              !property_tag.tolower().compare("inertiamatrix")) {
            utils::Error::check(
                !isInertiaSet,
                "Inertia matrix cannot be set twice. Please note that "
                "'Inertia', 'InertiaMatrix' and 'RadiiOfGyration' all set the "
                "inertia matrix.");
            ...
            utils::Error::check(
                isRTset == false, "RT should not appear before RTinMatrix");
            file.read(RTinMatrix);
          } else if (!property_tag.tolower().compare("rt")) {
            readRtMatrix(file, variable, RTinMatrix, RT);
            isRTset = true;
          } else if (
              !property_tag.tolower().compare("com") ||
              !property_tag.tolower().compare("centerofmass")) {
            readVector3d(file, variable, com);
          } else if (
              !property_tag.tolower().compare("forceplate") ||
              !property_tag.tolower().compare("externalforceindex")) {
            std::cout
                << "Please note that the 'externalforceindex' or 'forceplate' "
                   "tags were removed when the ExternalForceSet was "
                   "introduce.\n"
                << "Please consider removing these tag from your model file."
                << std::endl;
            ...
            if (!filePath.extension().compare("bioMesh")) {
              ...
              utils::Error::raise(
                  filePath.extension() + " is an unrecognized mesh file");
            }
            isMeshSet = true;
          } else if (!property_tag.tolower().compare("meshrt")) {
            utils::Error::check(
                isMeshSet,
                "mesh(es) or meshfile should be declared before meshrt");
            ...
            utils::Error::check(
                isMeshSet,
                "mesh(es) or meshfile should be declared before meshscale");
        ...
        if (!isRangeQSet) {
          ...
          for (size_t i = 0; i < trans.length() + rotLength; ++i) {
            if (!rot.compare("q") && i >= trans.length()) {
              QRanges.push_back(utils::Range(-1, 1));
            } else {
              QRanges.push_back(utils::Range());
            }
          }
        }
        if (!isRangeQdotSet) {
        ...
        model->AddSegment(
            ...
            characteristics,
            RT);
      } else if (!main_tag.tolower().compare("mesh_folder_prefix")) {
        file.read(meshFolderPrefix);
        ...
        while (file.read(var) && var.tolower().compare("endvariables")) {
          if (!var(0).compare("$")) {
            ...
            file.read(value);
            utils::Error::check(
                variable.find(var) == variable.end(),
        ...
        while (file.read(property_tag) &&
               property_tag.tolower().compare("endmarker"))
          if (!property_tag.tolower().compare("parent")) {
            // Dynamically find the parent number
            file.read(parent_str);
            parent_int =
                static_cast<size_t>(model->GetBodyId(parent_str.c_str()));
            // if parent_int still equals zero, no name has concurred
            utils::Error::check(
                model->IsBodyId(static_cast<unsigned int>(parent_int)),
                "Wrong name in a segment");
        ...
        while (file.read(property_tag) &&
          ...
          if (!property_tag.tolower().compare("parent")) {
            ...
            // If parent_int still equals zero, no name has concurred
            utils::Error::check(
                model->IsBodyId(model->GetBodyId(parent_str.c_str())),
                "Wrong name in a segment");
          } else if (!property_tag.tolower().compare("frommarkers")) {
            if (!firstTag) {
              utils::Error::raise(
                  "The tag 'fromMarkers' should appear first in the IMU " +
                  name);
            } else {
              fromMarkers = true;
            }
          ...
          if (fromMarkers) {
            if (!property_tag.tolower().compare("originmarkername")) {
              ...
              file.read(firstAxis);
            } else if (!property_tag.tolower().compare(
        ...
        if (fromMarkers) {
          std::vector<rigidbody::NodeSegment> allMarkerOnSegment(
              model->markers(parent_str));
          rigidbody::NodeSegment origin, axis1Beg, axis1End, axis2Beg, axis2End;
          bool isOrigin(false), isAxis1Beg(false), isAxis1End(false),
              isAxis2Beg(false), isAxis2End(false);
          ...
          if (!(isAxis1Beg && isAxis1End && isAxis2Beg && isAxis2End &&
                isOrigin)) {
            utils::Error::raise(
                "All the axes name and origin for the " + rtType + "(" + name +
                ") must be set and correspond to marker names "
                "previously defined on the same parent");
          }
          if (!(!axisToRecalculate.tolower().compare("firstaxis") ||
                !axisToRecalculate.tolower().compare("secondaxis"))) {
            utils::Error::raise(
                "The 'recalculate' option for " + rtType + "(" + name +
        ...
        utils::String axis("");
        while (file.read(property_tag) &&
               property_tag.tolower().compare("endcontact")) {
          if (!property_tag.tolower().compare("parent")) {
            // Dynamically find the parent number
            file.read(parentName);
            parent_int =
                static_cast<size_t>(model->GetBodyId(parentName.c_str()));
        ...
        while (file.read(property_tag) &&
          ...
          if (!property_tag.tolower().compare("predecessor")) {
            ...
            id_predecessor =
                static_cast<size_t>(model->GetBodyId(predecessor_str.c_str()));
            // If parent_int equals zero, no name has concurred
            utils::Error::check(
          ...
          if (!property_tag.tolower().compare("successor")) {
            ...
            id_successor =
                static_cast<size_t>(model->GetBodyId(successor_str.c_str()));
            // If parent_int equals zero, no name has concurred
            utils::Error::check(
                model->IsBodyId(static_cast<unsigned int>(id_successor)),
                "Wrong name in a segment");
        ...
        if (!contactType.tolower().compare("sphere")) {
          model->addSoftContact(
              rigidbody::SoftContactSphere(
                  pos,
                  radius,
                  stiffness,
                  damping,
...
#ifdef MODULE_ACTUATORS
        hasActuators = true;
        // The name of the actuator must correspond to the segment number to
        ...
        while (file.read(property_tag) &&
          ...
          if (!property_tag.tolower().compare("type")) {
            ...
            utils::Error::check(
                !str_direction.tolower().compare("positive") ||
            ...
            isT0Set = true;
              ...
              !property_tag.tolower().compare("pente") ||
        ...
        if (!type.tolower().compare("gauss3p")) {
          utils::Error::check(
              isDofSet && isDirectionSet && isTmaxSet && isT0Set && iswmaxSet &&
                  iswcSet && isaminSet && iswrSet && isw1Set && isrSet &&
                  isqoptSet,
              "Make sure all parameters are defined");
          actuator = new internal_forces::actuator::ActuatorGauss3p(
              int_direction,
              Tmax,
              T0,
              wmax,
          ...
          utils::Error::check(
              isDofSet && isDirectionSet && isTmaxSet,
              "Make sure all parameters are defined");
          actuator = new internal_forces::actuator::ActuatorConstant(
              int_direction, Tmax, dofIdx, name);
        } else if (!type.tolower().compare("linear")) {
          utils::Error::check(
              isDofSet && isDirectionSet && isSlopeSet && isT0Set,
              "Make sure all parameters are defined");
          actuator = new internal_forces::actuator::ActuatorLinear(
              int_direction, T0, slope, dofIdx, name);
        } else if (!type.tolower().compare("gauss6p")) {
          utils::Error::check(
              isDofSet && isDirectionSet && isTmaxSet && isT0Set && iswmaxSet &&
                  iswcSet && isaminSet && iswrSet && isw1Set && isrSet &&
                  isqoptSet && isFacteur6pSet && isr2Set && isqopt2Set,
              "Make sure all parameters are defined");
          actuator = new internal_forces::actuator::ActuatorGauss6p(
              int_direction,
              Tmax,
              T0,
              wmax,
          ...
          utils::Error::check(
              isDofSet && isThetaSet && isLambdaSet && isOffsetSet && isrSet &&
                  isqoptSet,
              "Make sure all parameters are defined");
          actuator = new internal_forces::actuator::ActuatorSigmoidGauss3p(
              int_direction, theta, lambda, offset, r, qopt, dofIdx, name);
        } else {
          utils::Error::raise(
              "Actuator do not correspond to an implemented one");
#ifdef _WIN32
          actuator = new internal_forces::actuator::ActuatorConstant(
              int_direction,
              Tmax,
        ...
        utils::Error::raise(
            "Biorbd was build without the module Internal forces but the model "
            "defines ones");
#endif  // MODULE_ACTUATORS
        ...
        while (file.read(property_tag) &&
          ...
          if (!property_tag.tolower().compare("originparent")) {
            ...
            size_t idx = static_cast<size_t>(
                model->GetBodyId(origin_parent_str.c_str()));
            // If parent_int still equals zero, no name has concurred
            utils::Error::check(
            ...
            size_t idx = static_cast<size_t>(
                model->GetBodyId(insert_parent_str.c_str()));
            // If parent_int still equals zero, no name has concurred
            utils::Error::check(
                model->IsBodyId(static_cast<unsigned int>(idx)),
        ...
        internal_forces::muscles::STATE_FATIGUE_TYPE dynamicFatigueType(
            internal_forces::muscles::STATE_FATIGUE_TYPE::
        ...
        // Read file
        while (file.read(property_tag) &&
               property_tag.tolower().compare("endmuscle")) {
          if (!property_tag.tolower().compare("musclegroup")) {
            ...
            if (!tp_state.tolower().compare("buchanan")) {
              ...
              utils::Error::raise(
                  property_tag + " is not a valid muscle state type");
            }
            ...
            while (file.read(subproperty_tag) &&
              ...
              if (!subproperty_tag.tolower().compare("type")) {
                ...
                if (!tp_fatigue_type.tolower().compare("simple")) {
                  dynamicFatigueType = internal_forces::muscles::
        ...
        internal_forces::muscles::MuscleGeometry geo(
            utils::Vector3d(
                origin_pos,
                name + "_origin",
                model->muscleGroup(static_cast<size_t>(idxGroup)).origin()),
            utils::Vector3d(
                insert_pos,
                name + "_insertion",
                model->muscleGroup(static_cast<size_t>(idxGroup)).insertion()));
        ...
        internal_forces::muscles::Characteristics characteristics(
            ...
            useDamping,
            maxShorteningSpeed);
        model->muscleGroup(static_cast<size_t>(idxGroup))
            .addMuscle(
                name,
                type,
                geo,
                characteristics,
                internal_forces::PathModifiers(),
                stateType,
                dynamicFatigueType);
        if (stateType == internal_forces::muscles::STATE_TYPE::BUCHANAN &&
            shapeFactor != 0) {
          ...
          static_cast<internal_forces::muscles::StateDynamicsBuchanan &>(state)
        ...
        utils::Error::check(
            insertion != "" && origin != "",
            "Insertion and origin of the ligament need to be defined.");
        internal_forces::Geometry geo(
            utils::Vector3d(origin_pos, name + "_origin", origin),
            utils::Vector3d(insert_pos, name + "_insersion", insertion));
        internal_forces::ligaments::LigamentCharacteristics characteristics(
            ligamentSlackLength, dampingFactor, maxShorteningSpeed);
        if (!type.tolower().compare("constant")) {
          utils::Error::check(
              std::isnan(force) == 0 && std::isnan(ligamentSlackLength) == 0,
              "Make sure all parameters are defined");
          ligament =
              std::make_shared<internal_forces::ligaments::LigamentConstant>(
                  force, name, geo, characteristics);
        } else if (!type.tolower().compare("linearspring")) {
          utils::Error::check(
              std::isnan(stiffness) == 0 &&
                  std::isnan(ligamentSlackLength) == 0,
              "Make sure all parameters are defined");
          ligament = std::make_shared<
              internal_forces::ligaments::LigamentSpringLinear>(
              stiffness, name, geo, characteristics);
          ...
          ligament = std::make_shared<
              ...
              stiffness, epsilon, name, geo, characteristics);
        } else {
          utils::Error::raise(
              "Ligament type do not correspond to an implemented one");
        }
        model->addLigament(*ligament);
#else   // MODULE_LIGAMENT
        utils::Error::raise(
            "Biorbd was build without the module Ligament but the model "
            "defines a ligament");
        ...
        // Verify that everything is there
        utils::Error::check(
            isTypeSet != 0, "PassiveTorque type must be defined");
        internal_forces::passive_torques::PassiveTorque *passiveTorque;
        if (!type.tolower().compare("constant")) {
          utils::Error::check(
              isDofSet && !std::isnan(Tconstant),
              "Make sure all parameters are defined");
          passiveTorque =
              new internal_forces::passive_torques::PassiveTorqueConstant(
                  Tconstant, dofIdx, name);
        } else if (!type.tolower().compare("linear")) {
          utils::Error::check(
              isDofSet && !std::isnan(slope) && !std::isnan(T0),
              "Make sure all parameters are defined");
          passiveTorque =
              new internal_forces::passive_torques::PassiveTorqueLinear(
                  T0, slope, dofIdx, name);
        } else if (!type.tolower().compare("exponential")) {
          utils::Error::check(
              isDofSet && isExponentialSet == 5,
              "Make sure all parameters are defined");
          passiveTorque =
              new internal_forces::passive_torques::PassiveTorqueExponential(
                  k1,
                  k2,
                  b1,
                  b2,
          ...
          utils::Error::raise(
              "PassiveTorque do not correspond to an implemented one");
        }
        model->addPassiveTorque(*passiveTorque);
        delete passiveTorque;
#else   // MODULE_PASSIVE_TORQUE
        utils::Error::raise(
            "Biorbd was build without the module Passive torque but the model "
            "defines one passive torque");
#endif  // MODULE_PASSIVE_TORQUE
      } else if (!main_tag.tolower().compare("viapoint")) {
        utils::String name;
        file.read(
            name);  // Name of via point... Eventually add the muscle group
        // Declaration of the variables
        utils::String parent("");
        utils::String muscle("");
        utils::String musclegroup("");
        ...
        while (file.read(property_tag) &&
          ...
          if (!property_tag.tolower().compare("parent")) {
            ...
            utils::Error::check(
                !isLigament,
                "Wrapping object cannot be define either for "
                "ligament and muscle.");
          } else if (!property_tag.tolower().compare("musclegroup")) {
            file.read(musclegroup);
          } else if (!property_tag.tolower().compare("ligament")) {
        ...
        if (!wrapType.tolower().compare("halfcylinder")) {
          utils::Error::check(
              radius > 0.0, "Radius must be defined and positive");
          utils::Error::check(length >= 0.0, "Length was must be positive");
        } else {
          utils::Error::raise(
              "Wrapping type must be defined (choices: 'halfcylinder')");
        }
        internal_forces::WrappingHalfCylinder cylinder(
            RT, radius, length, name, parent);
        if (isMuscle) {
          idxMuscleGroup = model->getMuscleGroupId(musclegroup);
          utils::Error::check(
              idxMuscleGroup != -1, "No muscle group was provided!");
    ...
    utils::String error_message(
        "Reading of file \"" + path.filename() + "." + path.extension() +
    ...
    if (subproperty_tag.compare("") &&
        subproperty_tag.find_first_of("end") != 0) {
...
std::vector<std::vector<utils::Vector3d>> Reader::readMarkerDataFile(
  ...
  for (size_t j = 0; j < nbMark; j++) {
    while (tp.compare("Marker")) {
      ...
      utils::Error::check(
          check, "Marker file error, wrong size of marker or intervals?");
    }

    size_t noMarker;
    file.read(noMarker);
    for (size_t i = 0; i <= nbIntervals;
...
std::vector<rigidbody::GeneralizedCoordinates> Reader::readQDataFile(
    const utils::Path &path) {
  // Open file
  ...
  for (size_t j = 0; j < nbIntervals + 1; j++) {
    while (tp.compare("T")) {
      bool check = file.read(tp);
      utils::Error::check(
          check, "Kin file error, wrong size of NDDL or intervals?");
    }
...
std::vector<utils::Vector> Reader::readActivationDataFile(
    const utils::Path &path) {
  // Open file
  // std::cout << "Loading kin file: " << path << std::endl;
#ifdef _WIN32
...
std::vector<utils::Vector> Reader::readTorqueDataFile(const utils::Path &path) {
  // Open file
  // std::cout << "Loading kin file: " << path << std::endl;
#ifdef _WIN32
  utils::IfStream file(
...
std::vector<utils::Vector> Reader::readGroundReactionForceDataFile(
    const utils::Path &path) {
  // Open file
  ...
  for (size_t j = 0; j < nbIntervals + 1; j++) {
    while (tp.compare("T")) {
      bool check = file.read(tp);
      utils::Error::check(
          check, "Grf file error, wrong size of NR or intervals?");
    }
...
void Reader::readViconForceFile(
    ...
    std::vector<std::vector<utils::Vector3d>> &force,   // Linear forces (x,y,z)
        ...
        &cop)  // Center of pressure (x,y,z) * number of pf
{
  ...
  while (!file.eof()) {
    // Put everything in temporary per platform (Mettre tout en temporaire par
    // plate-forme)
    ...
    while (!file.eof()) {  // return false if it fails
      ...
      // If line is empty, it's because we are done with the forces on this
      // platform
      if (!tp.compare("")) {
        break;
      }
      ...
      while (getline(ss, field, ',')) {
        ...
        // (since we require that the CSV contains nothing but floating-point
        // values)
      ...
      // Make sure that the line has the right number of elements(2 times, 3
      // cop, 3 forces, 3 moments)
      utils::Error::check(
          data.size() == 11,
      ...
      //        size_t subFrame = static_cast<size_t>(data[1]); // Subframes
      //        (not interesting)
...
std::vector<std::vector<utils::SpatialVector>> Reader::readViconForceFile(
  ...
  std::vector<std::vector<utils::Vector3d>> force;   // Linear forces (x,y,z)
  std::vector<std::vector<utils::Vector3d>> moment;  // Moments (x,y,z)
  std::vector<std::vector<utils::Vector3d>> cop;  // Center of pressure (x,y,z)
  readViconForceFile(path, frame, frequency, force, moment, cop);

  // Redispatch the values in a vector of SpatialTransform
  std::vector<std::vector<utils::SpatialVector>> st;  // nb of platform per time
...
std::vector<std::vector<utils::Vector3d>> Reader::readViconMarkerFile(
    const utils::Path &path,
  ...
  for (size_t i = 0; i < idx_init.size() - 1; ++i) {
    utils::String tp;
    for (size_t j = *(idx_init.begin() + i) + 1; j < *(idx_end.begin() + i);
         ++j) {
      tp.push_back(t.at(j));
    }
  ...
  for (int i = 0; i < static_cast<int>(markOrder.size()); ++i) {
    ...
    while (1) {
      ...
      //            utils::Error::check(cmp<MarkersInFile.size(), "The requested
      //            marker was not found in the file!");
  ...
  // now we'll use a stringstream to separate the fields out of the line (comma
  // separated)
  size_t cmpFrames(1);
  while (!file.eof()) {
...
rigidbody::Mesh Reader::readMeshFileBiorbdSegments(const utils::Path &path) {
  // Read a segment file
  ...
  size_t version(static_cast<size_t>(atoi(tp.c_str())));
  utils::Error::check(
      version == 1 || version == 2, "Version not implemented yet");
...
rigidbody::Mesh Reader::readMeshFileObj(const utils::Path &path) {
  // Read a bone file
...
rigidbody::Mesh Reader::readMeshFileVtp(const utils::Path &path) {
  ...
  utils::Error::check(
      err == tinyxml2::XML_SUCCESS, "Failed to load file " + filepath);
  ...
  utils::String field;
  {
    tinyxml2::XMLElement *pointsElement =
        polydata->FirstChildElement("Points")->FirstChildElement("DataArray");
  ...
  // Get the patches
  {
    tinyxml2::XMLElement *polysElement =
        polydata->FirstChildElement("Polys")->FirstChildElement("DataArray");
...
rigidbody::Mesh Reader::readMeshFileStl(const utils::Path &path) {
  // Read a bone file
  ...
  if (isBinary) {
    ...
    size_t nbTriangles =
        static_cast<size_t>(*((unsigned int *)nbTrianglesChar));
